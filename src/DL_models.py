...
Author: Tristan Waddington, Dimitri Iratchet
Date: dec 2024
"""
...
class TransformerModel(nn.Module):
    def __init__(
        ...
        )  # Map input to embedding dimension
        ...
        self.register_buffer(
            "positional_encoding",
            self.create_positional_encoding(input_size, embed_dim),
        )
    ...
    def forward(self, x):
        ...
        if len(x.shape) == 2:  # If input is [batch_size, seq_len]
            ...
            x = x.expand(
                -1, -1, self.embedding.in_features
            )  # Expand to [batch_size, seq_len, input_size]
        ...
        pos_enc = self.positional_encoding[:, : x.size(1), :].to(
            x.device
        )  # Crop positional encoding to match seq_len
